...
- [ ] **V21-05 — `corpus_hash()` is line-ending-sensitive (deploy bug, 2026-05-30):** `backend/cache/lineage.py::corpus_hash` is a raw `sha256(books.yaml bytes)`. Windows checkouts (CRLF) and Linux checkouts (LF) of the same git-stored file produce different hashes (`3f4fe940…` vs `d66003ec…`), which refused the SVM lineage on the first Railway deploy and missed every cache lookup. Workaround in commit `b265d5a`: `.gitattributes` pins `corpus/books.yaml text eol=crlf` so every platform materialises CRLF on checkout. Proper fix: normalise CRLF→LF inside `corpus_hash()`, re-stamp `lineage.json`'s `corpus_hash` field, re-key existing cache files (or full precompute rerun), publish a fresh data release, and remove the `eol=crlf` attribute.
